...
If the Web API becomes unreachable, the module switches to controlling the local Spotify app via AppleScript after about 6 seconds of failed polling, or immediately when a play, pause, next, previous, seek, volume, shuffle or repeat press fails on the network. That press is replayed locally, so it is not lost. If the request had already reached Spotify before the connection dropped, the replay can double a next or previous press. Play, pause, next, previous, seek, volume, fades, shuffle, repeat, play by ID (including a start position), play playlist, and bookmark resume keep working, and track info and album art still update. Playing a specific track or playlist while fully offline needs that content downloaded in the Spotify app. Starting a playlist at a specific track number is not available offline; playback starts from the beginning. Actions that need the internet (queue, like/unlike) log a warning and do nothing. The module probes the Web API every 10 seconds or so and switches back automatically once two probes in a row succeed, so an unstable connection cannot flip it back and forth mid-show.
