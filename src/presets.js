...
function getPresets() {
	...
	presets.push(
		...
		btn({
			...
			steps: [{ down: [{ actionId: 'playFadeIn', options: { duration: 5, target: 100 } }], up: [] }],
	...
	presets.push(
		btn({
			...
			steps: [{ down: [{ actionId: 'playPlaylist', options: { context: 'spotify:playlist:', offset: 1, position: '', shuffle: false } }], up: [] }],
			feedbacks: [],
		}),
		btn({
			category: 'Playlist',
			name: 'Play Album',
			style: { text: 'Album\\nPLAY', size: '14', color: white, bgcolor: blue },
			steps: [{ down: [{ actionId: 'playPlaylist', options: { context: 'spotify:album:', offset: 1, position: '', shuffle: false } }], up: [] }],
			feedbacks: [],
		}),
	)
	...
	presets.push(
		...
		btn({
			...
			style: { text: '', size: '18', color: white, bgcolor: black, png64: ICON_PREVIOUS, pngalignment: 'center:center' },
			steps: [{ up: [{ actionId: 'previous', options: {} }], down: [], 600: { actions: [{ actionId: 'movePosition', options: { seconds: -10 } }], options: { runWhileHeld: true } } }],
			feedbacks: [],
		}),
		btn({
			category: 'Transport Icons',
			...
			style: { text: '', size: '18', color: white, bgcolor: black, png64: ICON_NEXT, pngalignment: 'center:center' },
			steps: [{ up: [{ actionId: 'next', options: {} }], down: [], 600: { actions: [{ actionId: 'movePosition', options: { seconds: 10 } }], options: { runWhileHeld: true } } }],
			feedbacks: [],
		}),
	)
	...
	presets.push(
		btn({
			...
			feedbacks: [{ feedbackId: 'thisTrackPlaying', options: { track: 'spotify:track:' }, style: { color: white, bgcolor: green } }],
		}),
	)
